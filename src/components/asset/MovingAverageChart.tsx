...
export default function MovingAverageChart({ data, config, assetKey }: { data: ProcessedAsset; config: AssetConfig; assetKey: AssetKey }) {
  ...
  if (hasMM200) {
    datasets.push({ label: 'MM200', data: s.map((x, i) => ({ x: x.dateObj.getTime(), y: data.mm200?.[i] ?? null })), borderColor: '#ef4444', borderWidth: 1.5, pointRadius: 0, fill: false, borderDash: [8, 4] });
  }
  ...
  return (
    <>
      ...
      <div className={`grid ${hasMM200 ? 'grid-cols-2' : 'grid-cols-1'} gap-4 mt-4`}>
